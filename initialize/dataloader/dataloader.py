...
def load_authors():
    load_data_path = data_path.joinpath('authors.txt')
    read_file = open(load_data_path,"r",encoding="utf-8")
    line = read_file.readline()
    data = {}
    while(line):
...
def run():
    ...
    with open(out_data_path,'w',encoding='utf-8') as fout:
        for data in affiliations_data:
            for link in paper_author_affiliation_data:
                if link['affiliationid'] == data['id']:
                    # 加入作者
                    if can_add_author(link['authorid'],data['authors']):
                        data['authors'].append({
                            'id':link['authorid'],
                            'name':authors_data[link['authorid']]
                        })
                    # 加入文献
                    if can_add_paper(link['paperid'],data['pubs']):
                        paperdata = paper_data[link['paperid']]
                        data['pubs'].append({
                            'id':link['paperid'],
                            'title':paperdata['title']
                        })
                        data['n_citation']+=paperdata['n_citation']
                        data['n_pubs']+=1
